...
-- ------------------------------------------------------------------
SELECT product_name, list_price FROM northwind.products
WHERE list_price < 20.00 ORDER BY list_price DESC;
...
SELECT CASE discontinued  WHEN 1 then 'yes' else 'no'
END AS is_discontinued, COUNT(*) as product_count FROM northwind.products
...
FROM northind.products WHERE reorder_level > target_level;
SELECT reorder_level FROM northwind.products;
